# $Id: eutils.pm,v 1.7 2004/09/17 19:02:13 allenday Exp $
#
...
sub db {
    ...
    if($arg){
      my %ok = map {$_=>1} qw(pubmed pmc journals gds);
      if($ok{lc($arg)}){
        $self->{'db'} = lc($arg);
      } else {
        $self->warn( "invalid db '$arg', keeping value as ". ($self->{'db'} || $DEFAULT_DB) );
      }
    }
    return $self->{'db'};
}
...
sub get_by_id {
  my $self = shift;
  my $id = shift;
  my $db = $self->db || $DEFAULT_DB;
  $self->throw("must provide valid ID, not undef") unless defined($id);

  my $xml;
  if ( $self->db() eq 'gds' ) {
    $xml = get($EFETCH.'?rettype=brief&retmode=text&db='.$db.'&id='.$id);
  }
  else {
    $xml = get($EFETCH.'?rettype=abstract&retmode=xml&db='.$db.'&id='.$id);
  }
