...
The initial steps of feature selection, microarray design, microarray fabrication, experiment design, and biological sample processing, and image acquisition are described generally in Sections \ref{ArrayDesign}-\ref{Hybridization}.  A more exhaustive discussion of protocol surrounding these steps is given in \cite{wit2004}.  Following their acquisition, images are quantified, stored, and analyzed.  Challenges and findings related to these latter procedures are the subject of \emph{\dbthesis}.  They are thoroughly discussed in Section \ref{Quantification}.
...
\subsubsection{Feature Selection \& Microarray Design}
\label{ArrayDesign}
The first step in performing a microarray experiment is deciding which genes to assay on the microarray, how many genes to add, and how to arrange the genes on the microarray.  The process of selecting DNA sequences, or \emph{genome features}, which will represent the genes on the microarray is known as \emph{genome feature selection}.  The process of arranging features onto the microarray surface is known as \emph{array design}, and each uniquely identifiable region of the microarray design is known as an \emph{array feature}.

The surface of the microarray is bounded, and the microarray features on the microarray are not points -- they have dimensions.  This means that there is a limit to how many 
